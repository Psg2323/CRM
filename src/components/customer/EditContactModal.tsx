/**
 * EditContactModal 컴포넌트
 * -----------------------------------------------------
 * 주요 동작 요약:
 * - 연락처 정보를 수정하는 모달 폼입니다.
 * - 이름, 부서, 직책, 전화번호, 이메일, 연락일, 선호 채널, 키맨/임원 여부를 수정할 수 있습니다.
 * - '저장' 버튼 클릭 시 Supabase DB에 연락처 정보가 업데이트되고, 성공 시 부모 컴포넌트에 알림을 보냅니다.
 * - 입력값 검증(이름 필수), 저장 중 로딩 처리, 에러 발생 시 toast로 안내 등 사용자 경험을 고려했습니다.
 * - '취소' 버튼 또는 모달 바깥 클릭 시 모달이 닫힙니다.
 *
 * 상세 설명:
 * - contact prop이 바뀔 때마다 폼 상태가 해당 연락처 정보로 초기화됩니다.
 * - 이름이 비어 있으면 저장 시도 시 에러 메시지로 안내합니다.
 * - Supabase의 contacts 테이블 update 요청을 보내고, 성공하면 onContactUpdated를 호출합니다.
 * - 저장 중에는 버튼이 비활성화되고, 실패 시 toast로 에러 메시지를 안내합니다.
 * - Tailwind CSS 기반의 반응형 UI와 일관된 입력 컴포넌트를 사용합니다.
 */

import React, { useState, useEffect } from 'react';
// 모달(다이얼로그), 버튼, 입력창, 셀렉트, 라벨, 체크박스 등 UI 컴포넌트 불러오기
...
interface EditContactModalProps {
  ...
  contact: any; // 수정할 연락처 데이터
  onContactUpdated: () => void; // 연락처 수정 성공 시 실행할 함수(부모에게 알림)
}

// EditContactModal 컴포넌트 정의
const EditContactModal: React.FC<EditContactModalProps> = ({
  ...
  // contact prop이 바뀔 때마다 폼 상태를 해당 연락처 정보로 초기화
  useEffect(() => {
    if (contact) {
      setFormData({
        name: contact.name || '',
        ...
        is_keyman: contact.is_keyman === '1', // DB에서는 '1'/'0'으로 저장되어 있음
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Supabase에 contacts 테이블에서 해당 연락처를 업데이트
      const { error } = await supabase
        ...
        .eq('contact_id', contact.contact_id); // contact_id로 해당 연락처만 수정
